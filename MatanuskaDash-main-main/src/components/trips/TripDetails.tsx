...
const TripDetails: React.FC<TripDetailsProps> = ({ trip, onBack }) => {
  ...
  const handleGenerateSystemCosts = (systemCosts: Omit<CostEntry, 'id' | 'attachments'>[]) => {
    // Add all system costs
    systemCosts.forEach(costData => {
      addCostEntry(costData);
    });
    
    setShowSystemCostGenerator(false);
    alert(`${systemCosts.length} system cost entries have been automatically generated and added to the trip.`);
  };
  ...
  // NEW: Handle additional cost management
